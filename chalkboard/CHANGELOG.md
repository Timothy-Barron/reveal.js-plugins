...
- **Auto-activate drawing on pen touch**: Touching the slide with any pen (Apple Pencil, Surface Pen, etc.) automatically enables notes-on-slide drawing (equivalent to the `C` hotkey), so presenters don't need to press a hotkey before their first stroke. Only applies to notes-on-slide mode, not the chalkboard (`B`), and only for `pointerType === 'pen'` (touch/mouse input still requires the hotkey or toggle button).
